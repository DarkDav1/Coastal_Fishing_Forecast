...
- **`hourly_activity` wave curve**: `wave_height_m` / `swell_height_m` for each hour are taken directly from Open-Meteo **marine hourly** (`wave_height`, `swell_wave_height`) so the chart matches the API timestep; they are not inferred from the preview window rollup alone (which avoids a flat **1.0 m** line when the UI fell back to the engine’s swell default).
