...
- **Wave height curve**: `hourly_activity` fills `wave_height_m` / `swell_height_m` per hour from Open-Meteo marine hourly (`wave_height`, `swell_wave_height`, `wind_wave_height` when needed), matching timesteps in **Australia/Hobart** (not only `YYYY-MM-DDTHH` substring matching). If those are still missing, the row falls back to the window’s environment wave/swell. The web UI also draws a **sparse curve from scored day windows** when hourly marine fields are empty, with a short footnote instead of hiding the panel.
