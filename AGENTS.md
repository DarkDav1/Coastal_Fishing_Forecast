...
- **`POST /api/score-factors`** (Score factors): JSON body `{ lang, date, windows }`. Calls GitHub Models when **`GITHUB_TOKEN`** is set. Response `{ positive_factors: string[], negative_factors: string[], summary?: string, provider }`. Empty lists + `provider: fallback` → UI uses rule-based bullets in `dayScoreFactorsBullets()` in `App.tsx`. The web UI always **merges plain-language deduction hints** parsed from each window’s `formula.rules` (negative `score_delta`) and `negative_reason_tags` into the “What’s challenging” column, deduped with LLM bullets when both are present.
- Rule-based factors classify **`weather_trend.change_notes`** as **stress/challenging** unless the text matches easing/recovery wording; the snapshot “mild weather” bullet is omitted when stress notes exist so positives and negatives stay consistent.
