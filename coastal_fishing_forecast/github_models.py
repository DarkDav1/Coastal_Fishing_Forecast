...
def _score_factors_prompt(payload: Mapping[str, Any]) -> str:
    return (
        ...
        "- If payload.lang is zh, write clear Simplified Chinese with a neutral, product-style tone. "
        "Do not include English phrases except brand names or units such as kt, m, hPa, °C. "
        "Avoid casual wording such as '还可以', '看得出来', '好处理', '不是完全死水', or '打折'. "
        "Avoid literal translations like '存在感', '水体在中等程度上流动', or '近期暖段'. "
        "Explain what the condition means in concise condition-analysis language, not chatty advice.\n"
