...
class CacheAndApiTests(unittest.TestCase):
    ...
    def test_frontend_forecast_response_is_card_oriented(self) -> None:
        ...
        self.assertIn("classification", response["daily_forecast"][0]["windows"][0]["conditions"])
        self.assertIn("fish_outlook_score", response["daily_forecast"][0]["windows"][0])
        self.assertIn("trip_quality_score", response["daily_forecast"][0]["windows"][0])
        self.assertIn("waterbody_class", response["daily_forecast"][0]["windows"][0])
        self.assertIn("fish_profile", response["daily_forecast"][0]["windows"][0])
        self.assertIn("current_confidence", response["daily_forecast"][0]["windows"][0]["conditions"]["tide"])
        self.assertIn("water_temperature_signal", response["daily_forecast"][0]["windows"][0]["conditions"]["marine"])
        self.assertIsNotNone(response["daily_forecast"][0]["windows"][0]["conditions"]["tide"]["height_m"])
        self.assertIsNotNone(response["hourly_activity"][4]["tide_height_m"])
        self.assertIn("waterbody_class", response["hourly_activity"][4])
        self.assertIn("water_temperature_signal", response["hourly_activity"][4])
